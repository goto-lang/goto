...
func CreateModFile(ctx context.Context, modPath string) {
	...
	for _, f := range files {
		...
		if strings.HasSuffix(name, ".go") || strings.HasSuffix(name, ".goto") || f.IsDir() {
			empty = false
			break
		}
	}
...
func findModulePath(dir string) (string, error) {
	...
	for _, info := range list {
		if info.Type().IsRegular() && (strings.HasSuffix(info.Name(), ".go") || strings.HasSuffix(info.Name(), ".goto")) {
	...
	for _, info1 := range list {
		if info1.IsDir() {
			...
			for _, info2 := range files {
				if info2.Type().IsRegular() && (strings.HasSuffix(info2.Name(), ".go") || strings.HasSuffix(info2.Name(), ".goto")) {
